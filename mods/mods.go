...
type QueryMods[T any] []pgcraft.Mod[T]
...
type Moddable[T pgcraft.Expression] func(...pgcraft.Mod[T]) T

func (m Moddable[T]) WriteSQL(w io.Writer, start int) ([]any, error) {
	return m().WriteSQL(w, start)
}
...
type Values[Q interface {
	AppendValues(vals ...pgcraft.Expression)
}] []pgcraft.Expression
...
func (s Set[Q]) To(to any) pgcraft.Mod[Q] {
	return set[Q]{expr.OP("=", expr.Quote(s...), to)}
}

func (s Set[Q]) ToArg(to any) pgcraft.Mod[Q] {
