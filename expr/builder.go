...
import "github.com/codefrantic/pgcraft-go"
...
// Build an expression
func X[T pgcraft.Expression, B builder[T]](exp pgcraft.Expression, others ...pgcraft.Expression) T {
	var b B

	// Easy chain. For example:
	// X("a", "=", "b")
	if len(others) > 0 {
		exp = Join{Exprs: append([]pgcraft.Expression{exp}, others...)}
	}
...
func Not[T pgcraft.Expression, B builder[T]](exp pgcraft.Expression) T {
	var b B
	return b.New(Join{Exprs: []pgcraft.Expression{not, X[T, B](exp)}})
}
...
type Builder[T pgcraft.Expression, B builder[T]] struct{}

// prefix the expression with a NOT
func (e Builder[T, B]) Not(exp pgcraft.Expression) T {
	return Not[T, B](exp)
}

// Or
func (e Builder[T, B]) Or(args ...pgcraft.Expression) T {
	return X[T, B](Join{Exprs: args, Sep: " OR "})
}

// And
func (e Builder[T, B]) And(args ...pgcraft.Expression) T {
...
func (e Builder[T, B]) Group(exps ...pgcraft.Expression) T {
...
// quoted and joined... something like "users"."id"
func (e Builder[T, B]) Cast(exp pgcraft.Expression, typname string) T {
