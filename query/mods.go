...
func (di Distinct) WriteSQL(w io.Writer, start int) ([]any, error) {
	w.Write([]byte("DISTINCT"))
	return pgcraft.ExpressSlice(w, start, di.On, " ON (", ", ", ")")
}
...
func CrossJoin[Q Joinable](e any) pgcraft.Mod[Q] {
...
func (j JoinChain[Q]) Natural() pgcraft.Mod[Q] {
	jo := j()
	jo.Natural = true
...
func (j JoinChain[Q]) On(on ...pgcraft.Expression) pgcraft.Mod[Q] {
	jo := j()
...
func (j JoinChain[Q]) OnEQ(a, b pgcraft.Expression) pgcraft.Mod[Q] {
	jo := j()
...
func (j JoinChain[Q]) Using(using ...string) pgcraft.Mod[Q] {
	jo := j()
	jo.Using = using
...
func (c collation) WriteSQL(w io.Writer, _ int) ([]any, error) {
	if _, err := w.Write([]byte(" COLLATE ")); err != nil {
		return nil, err
	}
	expr.WriteQuote(w, c.name)
	return nil, nil
}
...
func (c CTEChain[Q]) As(q pgcraft.Query) CTEChain[Q] {
	cte := c()
	cte.Query = q
	return CTEChain[Q](func() clause.CTE {
		return cte
...
func (w *WindowChain[T]) FromPreceding(exp any) T {
	w.def.SetStart(pgcraft.ExpressionFunc(
		func(w io.Writer, start int) ([]any, error) {
			return pgcraft.ExpressIf(w, start, exp, true, "", " PRECEDING")
		}),
	)
	return w.Wrap
}
...
func (w *WindowChain[T]) FromFollowing(exp any) T {
	w.def.SetStart(pgcraft.ExpressionFunc(
		func(w io.Writer, start int) ([]any, error) {
			return pgcraft.ExpressIf(w, start, exp, true, "", " FOLLOWING")
		}),
	)
	return w.Wrap
}
...
func (w *WindowChain[T]) ToPreceding(exp any) T {
	w.def.SetEnd(pgcraft.ExpressionFunc(
		func(w io.Writer, start int) ([]any, error) {
			return pgcraft.ExpressIf(w, start, exp, true, "", " PRECEDING")
		}),
	)
